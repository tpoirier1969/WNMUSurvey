(function () {
  ...
  function init() {
    ...
    if (!access) return showAccessError("This follow-up link could not be connected to a completed main questionnaire in this browser. Use the private link shown after submitting the main questionnaire. Cross-device links will work after the production database is connected.");
    coreResponse = storage.getResponse(access.coreResponseId);
    if (!coreResponse) return showAccessError("The completed main questionnaire connected to this link is no longer available in this browser. The production version will store this connection in the approved database.");
  ...
  function renderLinkedSummary() {
    ...
    els.linkedCoreSummary.innerHTML = `<strong>Your main questionnaire is complete.</strong><span>Submitted ${escapeHtml(submitted)}</span><span>${escapeHtml(status)}</span><span>${escapeHtml(priorities.length ? priorities.join(", ") : "No programming priorities were selected")}</span><p>These follow-up answers will be connected to that response through a random pseudonymous respondent ID. You will not be asked to repeat the main questionnaire.</p>`;
  }
  ...
  function renderModuleGrid() {
    const submitted = storage.getFollowUpResponses();
  ...
  function renderTestTools() {
    const count = storage.getFollowUpResponses().filter((response) => response.accessId === access.accessId && response.followUpSchemaVersion === followUps.schemaVersion).length;
  ...
  function downloadResponses() {
    const responses = storage.getFollowUpResponses().filter((response) => response.accessId === access.accessId && response.followUpSchemaVersion === followUps.schemaVersion);
    storage.downloadJson(`wnmu-follow-up-responses-${new Date().toISOString().slice(0, 10)}.json`, { accessId: access.accessId, respondentId: access.respondentId, coreResponseId: access.coreResponseId, coreSchemaVersion: access.coreSchemaVersion, followUpSchemaVersion: followUps.schemaVersion, responses });
  }
  ...
  function renderQuestion(question, number) {
    ...
    let control;
    if (question.type === "textarea") control = `<textarea class="textarea-control" rows="5" data-question-id="${escapeAttr(question.id)}">${escapeHtml(value || "")}</textarea>`;
    else {
  ...
  function selectedCorePriorities() {
    ...
    const priorityQuestion = coreSurvey.stages.flatMap((stage) => stage.pages.flatMap((page) => page.questions)).find((item) => item.id === "program_category_priorities");
    const matrix = priorityQuestion?.optionsFromMatrix ? coreSurvey.stages.flatMap((stage) => stage.pages.flatMap((page) => page.questions)).find((item) => item.id === priorityQuestion.optionsFromMatrix) : null;
  ...
  function continuationUrl() {
    const url = new URL("follow-up.html", location.href);
    const hash = new URLSearchParams();
    hash.set("continue", access.token);
    url.hash = hash;
    return url.href;
