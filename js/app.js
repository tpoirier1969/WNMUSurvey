(function () {
  ...
  function showPanel(name) {
    ...
    if (name !== "hub") document.getElementById("surveyApp")?.scrollIntoView({ behavior: reducedMotion() ? "auto" : "smooth", block: "start" });
  }
  ...
  function renderPage(index, direction) {
    ...
    const questions = page.questions.filter((question) => isQuestionVisible(question));
    const content = `
    ...
    if (!els.sectionStage.innerHTML || direction === 0 || reducedMotion()) {
      ...
      transitionTimer = window.setTimeout(() => {
        ...
        requestAnimationFrame(() => requestAnimationFrame(() => { els.sectionStage.className = "section-stage"; }));
      }, 180);
    }
    ...
    window.setTimeout(() => {
      ...
      els.questionnairePanel?.scrollIntoView({ behavior: reducedMotion() ? "auto" : "smooth", block: "start" });
  ...
  function updateProgress(stage) {
    ...
    if (els.sectionPosition) els.sectionPosition.textContent = `Stage ${stage.number} of ${survey.stages.length} · Page ${state.currentPageIndex + 1} of ${stage.pages.length}`;
  }
  ...
  function validatePage(page) {
    ...
    const missing = page.questions.filter((question) => question.required && isQuestionVisible(question) && !hasValue(getQuestionValue(question)));
  ...
  function validateStage(stage) {
    ...
    for (let pageIndex = 0; pageIndex < stage.pages.length; pageIndex += 1) {
      const missing = stage.pages[pageIndex].questions.find((question) => question.required && isQuestionVisible(question) && !hasValue(getQuestionValue(question)));
      if (missing) {
        renderPage(pageIndex, pageIndex >= state.currentPageIndex ? 1 : -1);
        window.setTimeout(() => showValidationError(missing, "Please answer this required question before completing the stage."), 20);
        return false;
      }
    }
    return true;
  ...
  function showValidationError(question, message) {
    ...
    const block = els.sectionStage?.querySelector(`[data-question-block="${cssEscape(question.id)}"]`);
    if (block) {
  ...
  function handleInput(event) {
    ...
    if (["viewer_status", "viewing_methods", "children_role"].includes(questionId)) {
      ...
      renderPage(state.currentPageIndex, 0);
      const replacement = els.sectionStage.querySelector(`[data-question-id="${cssEscape(questionId)}"][value="${cssEscape(focusValue)}"]`);
  ...
  function enforceExclusive(target, block) {
    ...
    if (exclusive.includes(target.value)) boxes.forEach((box) => { if (box !== target) box.checked = false; });
  ...
  function renderQuestion(question, value, number) {
    ...
    if (question.type === "radio") control = `<div class="choice-grid">${options.map((option) => choiceMarkup(question, option, value, "radio")).join("")}</div>`;
    else if (question.type === "checkbox") control = `<div class="choice-grid">${options.map((option) => choiceMarkup(question, option, Array.isArray(value) ? value : [], "checkbox")).join("")}</div>`;
    else if (question.type === "select") control = `<select class="select-control" data-question-id="${escapeAttr(question.id)}"><option value="">Choose an option</option>${options.map((option) => `<option value="${escapeAttr(option.value)}" ${String(value) === String(option.value) ? "selected" : ""}>${escapeHtml(option.label)}</option>`).join("")}</select>`;
    else if (question.type === "text") control = `<input class="text-control" type="text" value="${escapeAttr(value || "")}" data-question-id="${escapeAttr(question.id)}" />`;
    else if (question.type === "textarea") control = `<textarea class="text-control textarea-control" rows="4" data-question-id="${escapeAttr(question.id)}">${escapeHtml(value || "")}</textarea>`;
    else if (question.type === "scale") control = `<div class="scale-row">${survey.scales[question.scale].map((option) => choiceMarkup(question, option, value, "radio", true)).join("")}</div>`;
    else if (question.type === "matrix") control = renderMatrix(question, value || {});
  ...
  function renderMatrix(question, values) {
    ...
    return `<div class="matrix-wrap"><table class="matrix-table matrix-card-table"><thead><tr><th scope="col">Area</th>${scale.map((option) => `<th scope="col">${escapeHtml(shortScaleLabel(option.label))}</th>`).join("")}</tr></thead><tbody>${question.rows.map((row) => `<tr><th scope="row">${escapeHtml(row.label)}</th>${scale.map((option) => `<td><label class="matrix-choice"><input type="radio" name="${escapeAttr(question.id)}__${escapeAttr(row.id)}" value="${escapeAttr(option.value)}" data-question-id="${escapeAttr(question.id)}" data-row-id="${escapeAttr(row.id)}" ${String(values[row.id]) === String(option.value) ? "checked" : ""} /><span class="matrix-visible-label">${escapeHtml(option.label)}</span><span class="sr-only">${escapeHtml(row.label)}: ${escapeHtml(option.label)}</span></label></td>`).join("")}</tr>`).join("")}</tbody></table></div>`;
  }
  ...
  function stageIsComplete(stage, candidateState) {
    ...
    if (allowsBlankNavigation()) return true;
    return !stage.pages.some((page) => page.questions.some((question) => question.required && isQuestionVisible(question, candidateState) && !hasValue(getQuestionValue(question, candidateState))));
  }

  function refreshCompletedStages(candidateState) {
    candidateState.completedStageIds = survey.stages.filter((stage) => stageIsComplete(stage, candidateState)).map((stage) => stage.id);
  }
  ...
  function stageHasAnswers(stage) {
    return stage.pages.some((page) => page.questions.some((question) => hasValue(getQuestionValue(question))));
  }
  ...
  function updateHubStatuses() {
    ...
    survey.stages.forEach((stage) => {
      ...
      document.querySelectorAll(`[data-stage-status="${cssEscape(stage.id)}"]`).forEach((element) => { element.textContent = text; });
    });
  }
  ...
  function updateResumeBlock() {
    ...
    if (draft && els.resumeDetails) {
      ...
      els.resumeDetails.textContent = draft.migratedFrom ? `Earlier answers recovered and updated · saved ${updated}` : `Saved ${updated}`;
    }
  }
  ...
  function saveDraftNow() {
    ...
    if (els.saveStatus) els.saveStatus.textContent = `Draft saved ${new Date(saved.updatedAt).toLocaleTimeString([], { hour: "numeric", minute: "2-digit" })}`;
    updateResumeBlock();
    updateHubStatuses();
  }
  ...
  function shortScaleLabel(label) {
    return ({
      "Not important": "Not", "Slightly important": "Slightly", "Moderately important": "Moderately",
      "Very important": "Very", "Not familiar enough to rate": "Not familiar",
      "Not interested": "Not", "Slightly interested": "Slightly", "Moderately interested": "Moderately",
      "Very interested": "Very", "Extremely interested": "Extremely"
    })[label] || label;
  }
